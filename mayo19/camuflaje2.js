...
/*He tenido que quitar una funcion que me daba problemas. Lo modifico de cara a la entrega*/
...
function put_random(x0, x1, y0, y1, color, prob){
    for (let x = x0; x <= x1; x +=100) {
        for (let y = y0; y <= y1; y +=100) {
            ...
            let radio = Math.round(Math.random()*30);
            if ((Math.random() >= prob)) {
...
function put_separacion(x, y, ancho, largo, color, color_borde, ancho_borde, solapamiento_permitido) {
  if (solapamiento_permitido) {
    vjcanvas.rectangle(x, y, ancho, largo, color, color_borde, ancho_borde);
  }
}
...
function main(){
    ...
    let largo = 100;
    let color = "white";
    let color_borde = "white";
    let ancho_borde = 1;
    let solapamiento_permitido = false;
    //vjcanvas.rectangle(x, y, ancho, alto, color, color_borde, ancho_borde);
    for (let i = 0; i < nfranjas-1; i++) {
      put_separacion(x, y, ancho, largo, color, color_borde, ancho_borde, solapamiento_permitido);
      y += 1000;
    }
};
